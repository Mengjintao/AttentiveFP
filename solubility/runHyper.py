...
#           if valid_MSE < 0.35:
                torch.save(model, model_path+'/model_'+prefix_filename+'_'+start_time+'_'+str(epoch)+'.pt')
...
# evaluate model
        best_model = torch.load(model_path+'/model_'+prefix_filename+'_'+start_time+'_'+str(best_param["valid_epoch"])+'.pt')     
...
#run(radius=2, T=2, fingerprint_dim=200, weight_decay=5, learning_rate=2.5, p_dropout=0.2)
...
np.random.seed(168)
gpgo = GPGO(gp, acq, run, param)
gpgo.run(max_iter=20, init_evals=2)
